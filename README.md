...
14. **触控容错（这块电容屏"点不动"的根因）**
   ...
   - **按下坐标可能是上一次触摸的旧值**：按下包里 BTN_TOUCH 与坐标的先后顺序不固定，
     ...
     （`pending_down` + `coords_pkt`，60ms 兜底），上层不会拿旧坐标做命中判定。
     注意这里有个坑：gslX680 最常见的报法是"坐标 + BTN_TOUCH 在**同一个包**里、
     坐标在前"，所以按下时**不能**清掉“本包有坐标”的标志，否则 DOWN 永远发不出去——
     普通点击全丢，而手指拖动后补发的 DOWN 会把一次滑动当成点击（表现为"左滑才有反应"）。
     标志只在包边界（EV_SYN）和抬起时清。
   ...
   屏幕键盘（AI 聊天）另有三个专门的问题，都修在 `apps/app_chat.c`：
   - **画键与判定的列数不一致**：第 4 行 z~m 画时按 7 列居中、判定按 8 列，
     整行错位 37px，"看着点 m、实际点到 n"；
   - **键间 6px 是死区**：落在缝里就完全没反应。现在键放大到 74×46、间隙 4px，
     并把坐标按"格"划分（间隙并入相邻键、上下再给 6px 容差），缝里也能出字；
   - **没有按下反馈**：手指按上去要等抬起才有反应，手感发虚。现在按下即高亮该键
     （高亮跟随手指移动），抬起才真正输入。
   布局只有一份（`kb_cell` / `kb_bar` 表），绘制与命中都从它取值，不会再错位。
   另外全项目做了一遍"命中区体检"：**凡是小于 9mm（约 48px）的目标一律外扩容差**
   （`ui_hit_pad`，见 core/ui.h）——返回键、对话框按钮、图库标签/垃圾桶/播放/箭头、
   快捷话题、顶栏胶囊、拼音候选条、相机与录像的左下缩略图、打砖块的暂停键都加了 4~8px。
   聊天输入栏改成**整条栏都可点开键盘**（以前只认那个 36px 高的小圆角框，
   栏内上下各 10px 和两侧留白都是死区，手指根本对不准）。
   体检结论：图库标签、音乐播放/切歌键、打砖块结算按钮的画与判定本来就一致，
   只是尺寸偏小，属于"加了容差更稳"的一类。
