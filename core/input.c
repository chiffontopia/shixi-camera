...
#define TAP_MOVE_MAX   22     /* 移动小于该值仍算点击 */
#define TAP_TIME_MAX   700    /* 点击最长按下时间(ms)，超过算长按 */
#define SWIPE_MIN      45     /* 滑动判定阈值(px) */
#define LONG_PRESS_MS  650    /* 长按触发时间 */
...
static void fill_up(UiEvent *e, int x, int y, int dur)
{
    ...
    if (adx < SWIPE_MIN && ady < SWIPE_MIN) {
        if (dur < TAP_TIME_MAX) e->tap = 1;
    } else if (adx >= ady) {
        e->swipe = dx > 0 ? SWIPE_RIGHT : SWIPE_LEFT;
    } else {
        e->swipe = dy > 0 ? SWIPE_DOWN : SWIPE_UP;
    }
    e->x0 = sx; e->y0 = sy;
}
